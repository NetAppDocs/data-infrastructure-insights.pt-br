...
'''
*Problema:* Após adicionar ou atualizar um ONTAP SVM Data Collector (Segurança de Carga de Trabalho), o tenant exibe um alerta de provisionamento excessivo ou violação de assinatura. A capacidade de storage bruto aumentou mesmo sem a adição de novo hardware.

*Experimente isto:* O mesmo storage pode ser contabilizado duas vezes — uma vez por um coletor de cluster ONTAP do Observability e novamente por um coletor SVM do Workload Security no mesmo cluster.

Compare a capacidade de storage bruto relatada pelos coletores Observability ONTAP e pelos coletores Workload Security SVM. Para cada coletor SVM, verifique se o cluster pai já está sendo monitorado por um coletor Observability ONTAP. Se estiver, somente a capacidade do cluster deve ser contabilizada no uso da assinatura.

Confirme se ocorreu alguma migração recente de SVM entre clusters. Após a migração de uma SVM, as métricas de capacidade podem permanecer associadas ao cluster anterior até que o Workload Security receba informações atualizadas do coletor.

Certifique-se de que o coletor de dados ONTAP SVM para a SVM migrada esteja em estado RUNNING. Se necessário, reinicie o coletor para que o Workload Security possa coletar as métricas corretas de cluster e capacidade após a migração.

Se a capacidade do cluster e a capacidade da SVM aparecerem no total, é provável que a capacidade esteja sendo contabilizada duas vezes. Isso geralmente acontece logo após adicionar, atualizar, reiniciar um coletor de SVM ou migrar uma SVM entre clusters.

Se o uso ainda exceder sua assinatura, abra um chamado de suporte da NetApp com o uso da assinatura antes e depois do alerta, e uma lista dos coletores de Observabilidade e SVM com suas respectivas capacidades de storage bruto relatadas.

{vazio}

'''
*Problema*: Um ou mais coletores de dados são exibidos como Degradados em vez de Em Execução, mesmo que o serviço do coletor esteja em execução.

*Experimente isto*: O SWS reporta "Degradado" quando uma ou mais conexões FPolicy em um nó são desconectadas. Isso não é uma interrupção, o coletor continua em execução, mas os eventos de auditoria desses nós podem ser perdidos até que a conectividade seja restaurada.

ONTAP estabelece uma conexão de canal de controle FPolicy a partir de cada nó que possui um LIF de dados operacional para a SVM. Um nó sem um LIF de dados local não pode se conectar. Veja:

* link:https://docs.netapp.com/us-en/ontap/nas-audit/node-fpolicy-server-communication-process-concept.html["Processo de comunicação do nó para o servidor ONTAP FPolicy externo"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/fpolicy-external-fpolicy-servers-concept.html["Como o ONTAP FPolicy funciona com servidores FPolicy externos"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/requirements-best-practices-fpolicy-concept.html["Requisitos, considerações e melhores práticas para configurar o ONTAP FPolicy"^]


Na interface do usuário, observe quais nós estão desconectados e verifique se esses nós possuem um LIF de dados para a SVM.

* Sem LIF de dados nos nós listados: desconsidere o status Degradado. O ONTAP pode mostrar o FPolicy desconectado com o motivo "No local lif present to connect to FPolicy server" quando nenhum LIF de dados local estiver presente; isso é esperado. Consulte link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/Fpolicy_server_not_connected_due_to_no_local_lif_present_on_one_or_more_nodes["KB: Servidor Fpolicy não conectado devido a "no local lif present""^]. Uma futura versão do SWS deixará de marcar o coletor como Degradado neste caso.
* LIF de dados presente nos nós listados: Investigue por que o FPolicy está desconectado (LIF inativo / cliente data-fpolicy ausente, rede/firewall ou reinicialização/failover temporário).
