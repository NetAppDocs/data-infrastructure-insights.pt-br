...
summary: Crie consultas personalizadas para pesquisar, filtrar e analisar seus ativos de infraestrutura e métricas com controle granular, agrupamento flexível e exibição de resultados personalizável. 
...
[role="lead"]
Crie consultas poderosas para pesquisar seus ativos de infraestrutura em um nível granular, com recursos flexíveis de filtragem, agrupamento e classificação. Desenvolva pesquisas direcionadas que ajudam você a identificar ativos específicos, analisar métricas de desempenho e organizar os resultados exatamente como você precisa.
