...
* O tipo ASSETS contém APIs de asset, query e search. Os atributos e métricas de DII asset às vezes são chamados de “odata”. As APIs para esses casos geralmente são acessadas por meio de /rest/v1/asset ou /rest/v1/query. Consulte a <<api-documentation-swagger,Documentação da API Swagger>> para mais informações.
+
...
* O tipo DATA INGESTION é usado para recuperar e gerenciar dados de ingestão e métricas personalizadas, como as provenientes de agentes Telegraf. Essas são às vezes chamadas de métricas “datalake” ou “lake”. As APIs para essas geralmente são acessadas por meio de /rest/v1/lake. Consulte a <<api-documentation-swagger,Documentação da API Swagger>> para mais informações.
...
Note que os tipos de API aos quais um usuário tem acesso também dependem da link:concept_user_roles.html["Função do usuário"] que ele possui em cada Data Infrastructure Insights feature set (Observabilidade, Workload Security, Reporting).
