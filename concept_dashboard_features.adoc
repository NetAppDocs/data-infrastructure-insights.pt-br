...
summary: Domine os recursos avançados do painel de controle, incluindo personalização de widgets, filtragem de consultas, configuração de variáveis, transformações de dados e opções de formatação para criar visualizações poderosas e flexíveis. 
...
[role="lead"]
Libere todo o potencial dos seus dashboards com recursos avançados e opções de personalização. Aprenda a configurar consultas de widgets, aplicar transformações de dados, usar variáveis para filtragem dinâmica e implementar formatação sofisticada para criar visualizações altamente eficazes e personalizadas para as suas necessidades de monitoramento de infraestrutura.
