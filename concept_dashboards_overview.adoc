...
summary: Crie painéis personalizados com widgets flexíveis para visualizar dados de infraestrutura, monitorar tendências de desempenho e construir visualizações operacionais adaptadas às suas necessidades específicas de monitoramento. 
...
[role="lead"]
Crie painéis personalizados que forneçam as informações operacionais de que você mais precisa. Escolha entre diversos tipos de widgets, incluindo tabelas, gráficos, indicadores e diagramas, para visualizar os dados da sua infraestrutura exatamente como você deseja, permitindo uma melhor tomada de decisão e um monitoramento mais eficaz.
