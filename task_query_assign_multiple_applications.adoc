...
summary: Gerencie com eficiência aplicativos e anotações em vários ativos usando operações em lote baseadas em consultas para atribuir ou remover esses itens sem a necessidade de configuração manual individual de cada ativo. 
...
[role="lead"]
Simplifique o gerenciamento de ativos utilizando consultas para realizar operações em lote em aplicações e anotações. Em vez de configurar manualmente cada ativo individualmente, você pode atribuir ou remover com eficiência várias aplicações e anotações em grupos de ativos em uma única operação.
