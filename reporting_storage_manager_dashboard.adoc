...
summary: Acesse insights centralizados de gerenciamento de storage com relatórios abrangentes sobre utilização de capacidade, tendências de desempenho e previsão de recursos para o gerenciamento proativo do data center. 
...
[role="lead"]
O painel de controle do Gerenciador de Armazenamento oferece insights centralizados sobre seu ambiente de storage por meio de relatórios e visualizações abrangentes. Monitore a utilização da capacidade, acompanhe as tendências de desempenho ao longo do tempo e compare as métricas atuais com as faixas aceitáveis para tomar decisões informadas sobre a manutenção do data center e o planejamento de recursos.
