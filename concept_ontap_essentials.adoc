...
O Painel de Segurança oferece uma visão instantânea da sua situação de segurança atual, exibindo gráficos para criptografia de hardware e software em volume, status de proteção contra adulteração de arquivos e métodos de autenticação de cluster. Os critérios de segurança são avaliados com base nas recomendações definidas no link:https://www.netapp.com/pdf.html?item=/media/10674-tr4569.pdf["* Guia de reforço de segurança da NetApp para ONTAP 9*"].

Selecione qualquer uma das opções de criptografia ou proteção contra adulteração de arquivos para analisar seu ambiente.
