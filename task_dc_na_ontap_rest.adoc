...
. Após o período de repouso, você deverá ver seus dados se estabilizarem à medida que o coletor REST continua a adquirir.
...
* Taxa de transferência – agregada de volumes internos.
* Gerenciamento – pode conter um hiperlink para a interface de gerenciamento do dispositivo. Criado programaticamente pela fonte de dados Data Infrastructure Insights como parte do relatório de inventário.
...
| Ao tentar criar um coletor de dados ONTAP REST, um erro como o seguinte é visto: Configuração: 10.193.70.14: API ONTAP REST em 10.193.70.14 não está disponível: 10.193.70.14 falhou ao GET /api/cluster: 400 Solicitação inválida | Isso provavelmente se deve a uma matriz ONTAP mais antiga (por exemplo, ONTAP 9.6) que não possui recursos de API REST. ONTAP 9.14.1 é a versão mínima do ONTAP suportada pelo coletor ONTAP REST. Respostas "400 Bad Request" devem ser esperadas em versões do ONTAP anteriores à REST. Para versões do ONTAP que suportam REST mas não são 9.14.1 ou posteriores, você pode ver a seguinte mensagem semelhante: Configuração: 10.193.98.84: ONTAP rest API em 10.193.98.84 não está disponível: 10.193.98.84: ONTAP rest API em 10.193.98.84 está disponível: cheryl5-cluster-2 9.10.1 a3cb3247-3d3c-11ee-8ff3-005056b364a7 mas não é da versão mínima 9.14.1. 
