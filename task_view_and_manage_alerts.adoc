...
Com as Janelas de Manutenção, você pode agendar períodos específicos de manutenção durante os quais as notificações de alerta são suprimidas para objetos e métricas que você escolher. Por exemplo, você pode suprimir as notificações de alerta acionadas por sistemas de storage específicos quando esses sistemas estiverem em um período de upgrade planejado.

Você pode escolher se deseja suprimir tanto os alertas quanto as notificações durante a janela, ou suprimir apenas as notificações, mas continuar detectando os alertas.

image:Maintenance_Windows_example.png["Exemplo de Janela de Manutenção, largura=400"]
...
. Acesse *Observability > Alerts > Maintenance Windows* e clique em *+ Maintenance Window*.
. Insira um *Nome* para a janela de manutenção. Para melhores resultados, escolha um nome significativo e específico.
. Selecione o *dispositivo de storage* ou o *atributo do objeto*.
. Filtre pelo *monitor ou monitores*. Caracteres curinga são aceitos.
. Selecione a *Hora de Início* e a *Hora de Término* para o período de manutenção.
. Escolha se deseja suprimir *alertas e notificações* ou *apenas notificações* (os alertas ainda são detectados durante a janela).
. Clique em *Salvar* para criar a janela de manutenção.
