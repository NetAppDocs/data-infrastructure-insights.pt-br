...
summary: Analise os monitores definidos pelo sistema para condições de métricas, registros, atributos e coleta de dados. 
---
...
[role="lead"]
O Data Infrastructure Insights inclui monitores definidos pelo sistema para métricas, logs, atributos e condições de coleta de dados. Os monitores de sistema disponíveis dependem dos coletores de dados presentes no seu tenant. Por isso, os monitores disponíveis no Data Infrastructure Insights podem mudar à medida que coletores de dados são adicionados ou suas configurações são alteradas.
...
Os monitores definidos pelo sistema são compostos por métricas ou atributos e condições predefinidos, bem como descrições padrão e ações corretivas, que não podem ser modificadas. Você _pode_ modificar a lista de destinatários de notificações para monitores definidos pelo sistema. Para visualizar as métricas ou atributos, condições, descrição e ações corretivas, ou para modificar a lista de destinatários, abra um grupo de monitores definidos pelo sistema e clique no nome do monitor na lista.
...
* *Infraestrutura CVO* e *Exemplos de Carga de Trabalho CVO* incluem monitores correspondentes para o Cloud Volumes ONTAP.
* O estado padrão varia de acordo com o monitor. Consulte as tabelas abaixo e verifique um monitor antes de ativar as notificações.
...
| O ventilador da prateleira falhou. | CRÍTICO | O ventilador de resfriamento indicado ou o módulo do ventilador da prateleira falhou.  Os discos na prateleira podem não receber fluxo de ar de resfriamento suficiente, o que pode resultar em falha do disco. | Execute as seguintes ações corretivas:…Verifique se o módulo do ventilador está totalmente encaixado e seguro.  OBSERVAÇÃO: O ventilador é integrado ao módulo de fonte de alimentação em algumas prateleiras de disco. Se o problema persistir, substitua o módulo do ventilador. Se o problema persistir, entre em contato com o suporte técnico da NetApp para obter assistência. 
...
| Período de carência da licença NVMe-oF ativo | AVISO | Este evento ocorre diariamente quando o protocolo NVMe over Fabrics (NVMe-oF) está em uso e o período de carência da licença está ativo.  A funcionalidade NVMe-oF requer uma licença após o término do período de carência da licença.  A funcionalidade NVMe-oF é desativada quando o período de carência da licença termina. | Entre em contato com seu representante de vendas para obter uma licença NVMe-oF e adicioná-la ao cluster ou remover todas as instâncias da configuração NVMe-oF do cluster. 


| Período de carência da licença NVMe-oF expirado | AVISO | O período de carência da licença NVMe over Fabrics (NVMe-oF) terminou e a funcionalidade NVMe-oF está desabilitada. | Entre em contato com seu representante de vendas para obter uma licença NVMe-oF e adicioná-la ao cluster. 


| Início do período de carência da licença NVMe-oF | AVISO | A configuração NVMe sobre Fabrics (NVMe-oF) foi detectada durante a atualização para o software ONTAP 9.5.  A funcionalidade NVMe-oF requer uma licença após o término do período de carência da licença. | Entre em contato com seu representante de vendas para obter uma licença NVMe-oF e adicioná-la ao cluster. 
...
| A realocação do pool de storage falhou. | CRÍTICO | Este evento ocorre durante a realocação de um agregado, quando o nó de destino não consegue alcançar os armazenamentos de objetos. | Execute as seguintes ações corretivas:…Verifique se o LIF intercluster está on-line e funcional usando o comando "network interface show".…Verifique a conectividade de rede com o servidor de armazenamento de objetos usando o comando "ping" no LIF intercluster do nó de destino.  …Verifique se a configuração do seu armazenamento de objetos não foi alterada e se as informações de login e conectividade ainda estão precisas usando o comando "aggregate object-store config show".…Como alternativa, você pode substituir o erro usando o parâmetro "override-destination-checks" do comando de realocação.…Entre em contato com o suporte técnico da NetApp para obter mais informações ou assistência. 
...
| Falha na recuperação de volume persistente do Kubernetes | O volume falhou na recuperação automática. |  | Aviso @ > 0 B 
...
| Loop de falha (CrashLoopBackOff) do pod do Kubernetes | O pod travou e tentou reiniciar diversas vezes. |  | Crítico @ > 3 
...
| Tarefa do Kubernetes falhou | O trabalho não foi concluído com sucesso devido a uma falha ou reinicialização do nó, exaustão de recursos, tempo limite do trabalho ou falha no agendamento do pod. | Verifique os logs de eventos do Kubernetes para detectar causas de falhas. | Aviso @ > 1 
...
| Volume Descoberto | Informativo | Esta mensagem ocorre quando um Volume Interno é descoberto. 


| Volume Modificado | Informativo | Esta mensagem ocorre quando um Volume Interno é modificado. 


| Nó descoberto | Informativo | Esta mensagem ocorre quando um nó de armazenamento é descoberto. 


| Nó removido | Informativo | Esta mensagem ocorre quando um nó de armazenamento é removido. 
...
Os monitores de atributos avaliam os atributos de configuração relatados para um objeto. Ao contrário dos monitores de métricas, eles não comparam uma métrica de série temporal com um limite numérico. Cada condição abaixo gera um alerta de _Aviso_ quando o atributo relatado corresponde a um estado não compatível ou indisponível.

Os monitores de métricas de segurança independentes anteriores do ONTAP Essentials foram substituídos pelos monitores de atributos consolidados *ONTAP Cluster Misconfigured* e *ONTAP Storage Misconfigured*. Os monitores correspondentes do CVO aplicam as mesmas verificações ao Cloud Volumes ONTAP.



==== Monitores de atributos do ONTAP

|===
| Nome do monitor | Grupo | Estado padrão | Condições | Ação corretiva 


| Cluster ONTAP mal configurado | Infraestrutura ONTAP | Ativo | Alertas quando uma ou mais destas 12 condições de segurança são detectadas: * AutoSupport HTTPS não está configurado * Web HTTP está habilitado * SSH não está seguro * O banner de login está desabilitado * A comunicação entre pares do cluster não está criptografada * O emissor do certificado é autoassinado * O administrador local padrão está habilitado * FIPS está desabilitado * O encaminhamento de logs não está criptografado * A hash de senha MD5 está habilitada * O Remote Shell (RSH) está habilitado * O Telnet está habilitado | Analise os atributos afetados no alerta. Desative protocolos inseguros e hash de senhas, proteja o SSH e a comunicação entre pares, habilite os controles de segurança necessários e substitua os certificados autoassinados de acordo com a política de segurança da sua organização. 


| ONTAP storage mal configurado | Infraestrutura ONTAP | Ativo | Alertas quando uma máquina virtual de armazenamento ONTAP está com o registro de auditoria desativado, o SSH não está seguro ou o banner de login está desativado. | Habilite o registro de auditoria da VM de armazenamento, proteja o SSH e configure um banner de login apropriado. 


| O cluster ONTAP não possui discos sobressalentes. | Infraestrutura ONTAP | Ativo | Alerta quando o cluster reporta que o número de discos sobressalentes está baixo. | Analise os discos sobressalentes por nó e adicione capacidade sobressalente compatível para manter a proteção de dados. 


| Serviço de armazenamento ONTAP indisponível | Exemplos de cargas de trabalho do ONTAP | Ativo | Alertas quando um serviço FCP ou iSCSI habilitado está indisponível em uma máquina virtual de armazenamento ONTAP. | Verifique o status da VM de armazenamento, do protocolo e do LIF de dados. Revise as mensagens recentes de integridade e eventos do ONTAP e restaure o serviço SAN afetado. 


| Nó ONTAP mal configurado | Exemplos de cargas de trabalho do ONTAP | Pausado | Alertas quando o Cisco Discovery Protocol (CDP) está desativado em um nó ONTAP. | Verifique a configuração do CDP no nó e o status da porta de rede e, em seguida, habilite o CDP onde for necessário para a descoberta de rede e o monitoramento da topologia. 
|===


==== Monitores de atributos do Cloud Volumes ONTAP

|===
| Nome do monitor | Grupo | Estado padrão | Condições | Ação corretiva 


| Cluster CVO mal configurado | Infraestrutura CVO | Ativo | Aplica as verificações de configuração de segurança do cluster descritas para *ONTAP Cluster Misconfigured* ao Cloud Volumes ONTAP. | Analise os atributos afetados e corrija as configurações de cluster inseguras ou não conformes relatadas. 


| Armazenamento CVO mal configurado | Infraestrutura CVO | Ativo | Alertas quando uma máquina virtual de armazenamento do Cloud Volumes ONTAP tem o registro de auditoria desativado, o SSH não está seguro ou o banner de login está desativado. | Habilite o registro de auditoria da VM de armazenamento, proteja o SSH e configure um banner de login apropriado. 


| Nó CVO mal configurado | Exemplos de carga de trabalho do CVO | Pausado | Alertas quando o CDP está desativado em um nó do Cloud Volumes ONTAP. | Verifique as configurações de CDP e de rede do nó e, em seguida, habilite o CDP onde for necessário. 
|===


==== Monitores de configuração do servidor NTP

Os seguintes monitores NTP continuam disponíveis como monitores independentes definidos pelo sistema para o ONTAP e o Cloud Volumes ONTAP.

|===
| Nome do monitor | Plataforma | Limite | Descrição do monitor | Ação corretiva 


| Nenhum servidor NTP está configurado | ONTAP | Aviso @ < 1 | O cluster não possui servidores NTP configurados. Para redundância e desempenho ideal, NetApp recomenda associar pelo menos três servidores NTP ao cluster. | Associe um servidor NTP ao cluster usando  `cluster time-service ntp server create -server <ntp_server>`. 


| A contagem do servidor NTP está baixa | ONTAP | Aviso @ < 3 | O cluster possui menos de três servidores NTP configurados. | Associe servidores NTP adicionais ao cluster usando  `cluster time-service ntp server create -server <ntp_server>`. 


| Nenhum servidor NTP está configurado no CVO | Cloud Volumes ONTAP | Aviso @ < 1 | O cluster não possui servidores NTP configurados. | Associe um servidor NTP ao cluster usando  `cluster time-service ntp server create -server <ntp_server>`. 


| O número de servidores NTP do CVO é baixo. | Cloud Volumes ONTAP | Aviso @ < 3 | O cluster possui menos de três servidores NTP configurados. | Associe servidores NTP adicionais ao cluster usando  `cluster time-service ntp server create -server <ntp_server>`. 
|===
...
| MetroCluster FC link do iniciador inativo | CRÍTICO | O adaptador iniciador FC está com defeito. | 1. Certifique-se de que o link do iniciador FC não foi adulterado. 2.  Verifique o status operacional do adaptador iniciador FC usando o comando "system node run -node local -command storage show adapter". 


| MetroCluster FC-VI Link de interconexão inativo | CRÍTICO | O link físico na porta FC-VI está offline. | 1. Certifique-se de que o link FC-VI não foi adulterado. 2.  Verifique se o status físico do adaptador FC-VI é "Ativo" usando o comando "metrocluster interconnect adapter show". 3.  Se a configuração incluir switches de malha, certifique-se de que eles estejam devidamente cabeados e configurados. 
