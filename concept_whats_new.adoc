...
=== Visualizações do Assistente de IA

Transforme dados complexos em insights gráficos instantaneamente. Peça ao Assistente de IA para criar um painel e ele visualizará a resposta com gráficos e tabelas interativos, ajudando você a identificar tendências e resolver problemas mais rapidamente sem a necessidade de criar gráficos manualmente.

image:ai_assistant-column_graph.png["Visualização do Assistente de IA"]



[[enhanced-vm-analyzer-filtering]]
=== Filtragem aprimorada do VM Analyzer

Uma nova opção na visualização do Analisador de VMs permite que você oculte volumes montados sem um datastore, oferecendo uma visualização mais limpa para simplificar a solução de problemas em VMs.

image:vm_analyzer_datastore_only_view.png["controle deslizante de visualização somente do datastore"]



[[ui-update-user-data-edit-mode]]
=== Atualização da interface do usuário: modo de edição de dados do usuário

Mantenha as informações dos ativos precisas enquanto reduz o risco de alterações não intencionais. Anotações e atributos (ou seja, "Dados do Usuário") agora aparecem em modo somente leitura por padrão, assim você pode revisar os detalhes primeiro e escolher quando estiver pronto para editá-los.

Edite somente quando necessário. Em um painel deslizante, expanda os detalhes do ativo e selecione o ícone de lápis. A página inicial do ativo será aberta no modo de edição, onde você pode atualizar as informações e selecionar Salvar quando terminar.

Você também pode abrir a página inicial do ativo diretamente e selecionar Editar no canto superior direito.

image:UserDataEdit-Slideout.png["Editar dados do usuário no painel deslizante, ícone de lápis, largura=400"]

image:UserDataEdit-LP.png["Modo de edição da página inicial, largura=200"]
...
=== Nova página inicial do assistente de IA

O Assistente de IA da DII agora conta com uma página inicial dedicada, colocando a análise inteligente de infraestrutura em destaque. Inicie novas conversas instantaneamente, retome sua última sessão com um clique ou comece rapidamente com sugestões de prompts.

image:AI_dashboard_page_default.png["Painel de IA padrão, largura=800"]

Acesse a página inicial pelo *Assistente de IA* no menu de navegação ou abra o Assistente de IA de qualquer lugar usando o ícone de "estrelas" no canto superior esquerdo.

image:AI_dashboard_page_menu.png["Menu de navegação do assistente de IA, largura=600"] image:AI_assistant_access_button.png["Acesso ao assistente de IA de qualquer lugar, width=300"]
...
=== Compartilhe atualizações importantes com mensagens de login

Mantenha sua equipe informada e alinhada com mensagens de login personalizáveis. Comunique anúncios importantes, cronogramas de manutenção ou atualizações de políticas diretamente aos usuários quando eles fizerem login. Com suporte a markdown, você pode formatar as mensagens para maior clareza e garantir que informações críticas cheguem a todos os usuários do DII. Contas com privilégios de administrador podem encontrar a configuração no menu Admin, em Notificações.



[[more-workspace-less-clutter]]
=== Mais espaço de trabalho, menos bagunça

O menu de navegação do DII tem um visual totalmente novo, agora combinando com as cores e o comportamento do Console para uma experiência mais unificada. O menu principal, antes estático, será recolhido por padrão para um espaço de trabalho mais focado e um uso mais eficiente do espaço na tela. Basta clicar no ícone de menu (☰) no canto superior esquerdo para acessá-lo.



[[expand-device-visibility-with-intelligent-ai-assistance]]
=== Amplie a visibilidade do dispositivo com assistência inteligente de IA

Acelere a integração de novos dispositivos com a identificação de dispositivos baseada em IA. Data Infrastructure Insights analisa seu ambiente e sugere regras de Resolução Automática de Dispositivos, ajudando você a identificar e gerenciar mais dispositivos rapidamente. Basta revisar e aprovar as regras geradas por IA e Data Infrastructure Insights cuida do resto, melhorando a visibilidade e o gerenciamento de dispositivos em toda a sua infraestrutura.



[[a-smarter-ai-assistant-for-global-teams]]
=== Um assistente de IA mais inteligente para equipes globais
...
=== Novos monitores para alterações de atributos de infraestrutura

Agora, os Monitores de Atributos estão disponíveis para alertar automaticamente sua equipe quando ativos forem movidos entre data centers, um pool de storage não estiver criptografado ou ocorrerem modificações em atributos. Isso ajuda a manter as configurações padrão e ajuda a garantir que nada passe despercebido durante novas implantações ou alterações.

image:attribute_monitor_in_list.png["Monitores de atributos são adicionados aos tipos de monitor"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== Crie alertas e webhooks mais inteligentes com mensagens dinâmicas e ricas em contexto

Ao criar um monitor ou webhook, basta digitar "%" na descrição do alerta ou na URL, cabeçalho ou corpo do webhook para selecionar em uma lista preenchida dinamicamente de atributos e anotações de objeto disponíveis. Forneça o contexto adequado aos seus alertas para melhorar a clareza, aprimorar a integração do webhook e acelerar os tempos de resposta.

image:attribute_list_example.png["Lista de atributos na descrição do monitor, largura=400"]
...
=== DII conhece o seu ambiente melhor do que nunca.

Aprimoramos a forma como a IA do Data Infrastructure Insights entende sua configuração. Utilizando um contexto mais rico sobre seu uso atual do serviço, a configuração e os padrões de desempenho antes de responder. O resultado é uma orientação inicial que reflete seu ambiente, em vez de um ponto de partida genérico, com menos perguntas adicionais necessárias para chegar a uma resposta útil.

Por exemplo, navegue até um dispositivo de storage e o Assistente de IA fornecerá respostas com um clique para perguntas como:

* Como está a tendência de utilização da capacidade deste storage e quais StoragePools estão consumindo mais espaço?
* Você pode me mostrar a tendência de utilização da capacidade deste dispositivo nas últimas 24 horas?
* Quais volumes estão consumindo mais espaço neste storage?
...
=== AI Assistant agora está disponível de forma geral

O Assistente de IA do Data Infrastructure Insights agora está disponível para todos os clientes com sites do DII hospedados na AWS.

Observe que o Assistente de IA nos sites do Azure ainda está em versão prévia. Fique atento a este espaço para anúncio da disponibilidade geral.



[[infrastructure-insights-features-removed]]
=== Recursos do Infrastructure Insights removidos

Os recursos "Infrastructure Insights" do DII, como _Shared Resources Under Stress_, _Kubernetes Namespaces Running out of Space_ e _Reclaiming ONTAP Cold Storage_, não estão mais disponíveis no DII.
...
=== Respostas inteligentes instantâneas sobre sua infraestrutura

Conheça o novo parceiro inteligente da sua infraestrutura: O link:ai_for_dii_overview.html["*Data Infrastructure Insights Assistente de IA*"] transforma a maneira como você interage com seus dados, convertendo consultas complexas em conversas simples. Descubra instantaneamente padrões ocultos, acelere a solução de problemas e tome decisões baseadas em dados com confiança. Obtenha respostas instantâneas para perguntas como:
...
A versão prévia do DII AI Assistant já está disponível para a maioria dos clientes com serviço hospedado em regiões da AWS; a região Azure estará disponível em breve. link:concept_preview_features.html["Saber mais"] sobre os recursos de prévia do Data Infrastructure Insights.

image:ai_assistant_icon.png["Ícone do AI Assistant"]
...
==== Personalize os monitores do sistema mais rapidamente, sem duplicar

Modifique os monitores de sistema padrão diretamente para atender às suas necessidades—altere nomes, filtros e agrupamentos em um único local. Não é necessário criar monitores duplicados para ajustes simples. Um banner útil lembra você quais monitores foram personalizados, mantendo total visibilidade da sua configuração de monitoramento.



==== Simplifique a gestão de monitores com ações em massa

Gerencie vários monitores simultaneamente por meio de Ações em Massa. Ajuste rapidamente as configurações de Monitoramento de Integridade para grupos de monitores, economizando tempo e reduzindo a complexidade. Atualize sua estratégia de monitoramento de forma abrangente em segundos, não em minutos.



==== Concentre suas pontuações de integridade da infraestrutura no que mais importa

Exclua objetos específicos das suas pontuações de Infrastructure Health para manter a precisão e a relevância. Ao visualizar o painel de Infrastructure Health, remova instantaneamente os objetos que deseja reduzir a prioridade sem modificar a configuração do monitor. Isso concentra seus alertas nos elementos de infraestrutura que impulsionam as prioridades do seu negócio.

image:health_monitoring_exclude_object.png["excluir objeto do monitoramento de saúde"]
...
Os seguintes sistemas operacionais Linux são atualmente suportados com Data Infrastructure Insights link:concept_acquisition_unit_requirements.html["Unidades de Aquisição"]:

* AlmaLinux 9.5 e 10
* CentOS (64-bit): Stream 9
* Debian (64-bit): 11 a 13
* OpenSUSE Leap 15.2 a 15.6, 16
* Oracle Enterprise Linux (64-bit): 8.10, 9.2, 9.4, 9.6, 10
* Red Hat (64-bit): 8.10, 9.2, 9.4, 9.6, 10
* Rocky 9.2 a 9.6
* SUSE Linux Enterprise Server 15 SP4 até 15 SP7, 16
* Ubuntu Server: 20.04, 22.04, 24.04 LTS
* SELinux nas plataformas acima
...
O Reclaim ONTAP storage frio Insight agora oferece suporte a FlexGroups e agora está disponível para todos os clientes.
...
| Namespaces do Kubernetes ficando sem espaço | O recurso _Kubernetes Namespaces Running Out of Space_ oferece uma visão das cargas de trabalho em seus namespaces do Kubernetes que correm o risco de ficar sem espaço, com uma estimativa do número de dias restantes antes que cada espaço fique cheio. 


| Recurso compartilhado sob estresse | O insight _Shared Resource Under Stress_ utiliza IA/ML para identificar automaticamente onde a disputa por recursos está causando degradação de desempenho em seu ambiente, destaca quaisquer cargas de trabalho afetadas por isso e fornece ações recomendadas para remediar, permitindo que você resolva problemas de desempenho mais rapidamente. 
...
Cloud Insights destaca regularmente uma série de novos e interessantes recursos em versão prévia. Se você tiver interesse em experimentar um ou mais desses recursos, entre em contato com sua equipe de vendas da NetApp para obter mais informações.
...
| Recurso compartilhado sob estresse | O insight "Recursos Compartilhados Sob Estresse" utiliza IA/ML para identificar automaticamente onde a disputa por recursos está causando degradação de desempenho em seu ambiente, destaca quaisquer cargas de trabalho afetadas por isso e fornece ações recomendadas para remediação, permitindo que você resolva problemas de desempenho mais rapidamente. 
|===
...
'''nt1.png[Gerenciamento de Widget Unit de Tabela]
