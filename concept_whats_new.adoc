...
O recurso *VM Analyzer* do DII oferece uma visão abrangente e completa dos seus ambientes virtualizados. O VM Analyzer agiliza a solução de problemas mapeando todo o caminho, das máquinas virtuais ao armazenamento, permitindo que os usuários identifiquem rapidamente problemas de desempenho em discos virtuais, VMs, hosts, datastores, volumes e armazenamento. Com uma visão intuitiva da topologia, filtragem e métricas de desempenho detalhadas, os administradores podem identificar facilmente gargalos, analisar desequilíbrios de tráfego e otimizar o desempenho geral do armazenamento.

image:vm_analyzer_example_with_panel.png["Mapa de topologia de um sistema de storage do VM Analyzer"]



=== Suporte ao sistema operacional

Os seguintes sistemas operacionais agora são suportados com unidades de aquisição do Data Infrastructure Insights, além dos seguinteslink:concept_acquisition_unit_requirements.html["já suportado"]:
