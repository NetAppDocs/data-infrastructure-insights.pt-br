...
| Erros "ImagePullBackoff" vistos | Esses erros podem ocorrer se você tiver um repositório docker personalizado ou privado e ainda não tiver configurado o Kubernetes Monitoring Operator para reconhecê-lo corretamente. <<using-a-custom-or-private-docker-repository,Ler mais>> sobre configurar para repositório personalizado/privado. 
